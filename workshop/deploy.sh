...
exp=$(ibmcloud cs cluster-config $CLUSTER_NAME | grep export)
if [ $? -ne 0 ]; then
...
if [ $? -eq 0 ]; then
  ibmcloud cs cluster-service-unbind $CLUSTER_NAME default tone
fi
...
ibmcloud cs cluster-service-bind $CLUSTER_NAME default tone
